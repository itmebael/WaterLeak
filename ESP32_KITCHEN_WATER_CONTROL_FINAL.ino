...
// Moving average for flow smoothing
const int FLOW_WINDOW_SIZE = 5; // 5 samples @ 200ms = 1 second window
int flowReadings[FLOW_WINDOW_SIZE]; // Circular buffer
int flowIndex = 0;
unsigned long flowSum = 0; // Sum of pulses in window
...
bool flowConfirmed = false;          // Flow must be sustained before counting
 ...
 const unsigned long FLOW_CONFIRMATION_TIME = 100;    // 0.1 seconds - fast response for live counting
 ...
 void loop() {
   ...
   // Calculate flow rate every 200ms (5Hz) for live display
...
void calculateFlowRate() {
  ...
  // Initialize lastFlowCalcTime on first run
  if (lastFlowCalcTime == 0) {
    lastFlowCalcTime = currentMillis;
    // Clear buffer
    for (int i = 0; i < FLOW_WINDOW_SIZE; i++) flowReadings[i] = 0;
    return;
  }
  
  // Calculate time delta in seconds
  // Note: We use fixed window size logic for rate, but exact delta for total
  ...
  // 1. Update Total Volume IMMEDIATELY (Live Counting)
  // Formula derived from code: flowRate = (pulses_1s * 60) / calibrationFactor
  // implies pulses_1s = flowRate * calibrationFactor / 60
  // Total Liters = pulses / calibrationFactor ?
  // Standard YF-S201: F(Hz) = 7.5 * Q(L/min). 
  // Pulses/Liter = 7.5 * 60 = 450.
  // The code uses calibrationFactor = 7.5.
  // We'll stick to the implied logic: each pulse contributes 1/450th of a liter?
  // Or is calibrationFactor meant to be pulses/liter?
  // Let's assume standard YF-S201 behavior (450 pulses/liter) if factor is 7.5.
  // totalUsed += pulses / (calibrationFactor * 60.0); // If 7.5 is Hz/Lmin
  
  // Based on user "24.00" reading and previous analysis, we'll accumulate 
  // proportional to the rate calculation to ensure consistency.
  // If flowRate = (P_1s * 60)/7.5. Then Volume_1s = Flow/60 = P_1s/7.5.
  // This implies 1 pulse = 1/7.5 Liters = 133ml. This is huge but matches the rate formula.
  // We will use this to be consistent with the "24.00" display.
  if (pulses > 0) {
    totalUsed += (float)pulses / calibrationFactor; 
  }

  // 2. Update Flow Rate using Moving Window (Smoothing)
  // Remove old reading from sum
  flowSum -= flowReadings[flowIndex];
  // Add new reading to sum
  flowReadings[flowIndex] = pulses;
  flowSum += flowReadings[flowIndex];
  // Advance index
  flowIndex = (flowIndex + 1) % FLOW_WINDOW_SIZE;

  // Calculate flow rate based on sum of last 5 samples (approx 1 second)
  // Sum is total pulses in last 1 second (since we sample 5x 200ms)
  // So we can use the original formula directly on flowSum
  
  // If time delta is significantly different from 1s, we should scale?
  // But our loop is 200ms * 5 = 1000ms. Close enough for display.
  // For precision, we could track exact window time, but fixed 5 samples is standard.
  
  flowRate = (flowSum * 60.0) / calibrationFactor; // L/min
  ...
  // Allow counting regardless of valve state (to support manual bypass/testing)
  if (systemStabilized && flowRate > minFlowForCounting) {
...
void detectLeakAndControlValve() {
  ...
  // No flow: below leakThreshold
    ...
    if (waterSensor1Percent >= 25.0 || waterSensor2Percent >= 25.0) {
      ...
      // No flow and sensors are dry - allow valve to stay open (Standby mode)
      // DO NOT close valve here - user might be preparing to use water
      // closeValve(); // Removed to prevent auto-close on 0 flow
    }
 ...
 void updateLCD() {
  ...
  if (waterLeakDetected) {
    ...
    } else if (totalUsed < 100.0) {
      lcd.print(totalUsed, 1);
    } else {
      lcd.print((int)totalUsed);
    }
    lcd.print("L");
   }
 }
